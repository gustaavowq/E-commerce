...
export function signAccessToken(payload: { userId: string; role: 'CUSTOMER' | 'ADMIN' }): string {
  const opts: SignOptions = {
    expiresIn: env.JWT_EXPIRES_IN as SignOptions['expiresIn'],
    issuer: 'miami-store',
    audience: 'miami-store-app',
  }
...
// Refresh tokens são opacos (não JWT) — guardamos hash no banco e enviamos
...
export function expiresInToMs(expiresIn: string): number {
  ...
  const multipliers: Record<'s' | 'm' | 'h' | 'd', number> = { s: 1000, m: 60_000, h: 3_600_000, d: 86_400_000 }
