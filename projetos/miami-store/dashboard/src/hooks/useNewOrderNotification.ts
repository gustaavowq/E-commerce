...
//   - Toca som (silencioso se browser bloquear autoplay — é OK, não trava)
...
export type PendingCounts = {
  ordersToday: number
  pendingActions: number  // PENDING_PAYMENT + PREPARING (do summary já existente)
}
...
async function pollOnce() {
  try {
    const summary: any = await adminDashboard.summary(1)  // 1 dia: foco em hoje
    emit({
      ordersToday:    Number(summary?.ordersToday    ?? summary?.orders?.today    ?? 0),
      pendingActions: Number(summary?.pendingActions ?? summary?.pending          ?? 0),
    })
  } catch {
